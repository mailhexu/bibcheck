...
export interface OriginalFields {
  [entryId: string]: Record<string, string>;
}
...
export function useApiIntegration() {
  ...
  const [originalFields, setOriginalFields] = useState<OriginalFields>({});

  // Save original field values before applying any changes
  const saveOriginalFields = (entry: BibTeXEntry) => {
    if (!originalFields[entry.id]) {
      setOriginalFields(prev => ({
        ...prev,
        [entry.id]: { ...entry.fields }
      }));
      console.log(`🔄 Saved original fields for ${entry.id}:`, entry.fields);
    }
  };

  // Restore original field value
  const restoreOriginalField = (entry: BibTeXEntry, field: string): string | undefined => {
    const originalValue = originalFields[entry.id]?.[field];
    console.log(`🔄 Restoring original field for ${entry.id}.${field}:`, originalValue);
  ...
  const fetchMissingFields = async (
    ...
    try {
      for (let i = 0; i < entries.length; i++) {
        ...
        // Save original fields before any modifications
        saveOriginalFields(entry);
        ...
        if (!entry.fields.doi && entry.fields.arxiv) {
          ...
          try {
            ...
            console.error('Error fetching DOI from arXiv:', error);
          }
        }
        ...
        if (!updatedEntry.fields.doi && (entry.fields.title || entry.fields.author || entry.fields.journal)) {
          ...
          try {
            ...
            console.error('Error fetching DOI from CrossRef:', error);
          }
        }
      ...
      console.error('Error in fetchMissingFields:', error);
      setError(error instanceof Error ? error.message : 'An error occurred while fetching missing fields');
    } finally {
      setLoading(false);
      setProgress(null);
    }
  ...
  const fetchMissingFieldsForEntry = async (
    ...
    try {
      ...
      console.error('Error in fetchMissingFieldsForEntry:', error);
      setError(error instanceof Error ? error.message : 'An error occurred while fetching missing fields');
    } finally {
      setProgress(null);
    }
